...
data Lens s t a b =
  Lens (forall f. Functor f => Optic (->) f s t a b)

-- 1. Choice p =>
-- 2. Applicative f =>
----
-- prism ~ (Choice p, Applicative f) => (p a (f b)) -> (p s (f t))
data Prism s t a b =
  Prism (forall p f. (Choice p, Applicative f) => Optic p f s t a b)

-- 1. p ~ (->)
-- 2. Applicative f =>
----
-- traversal ~ Applicative f => (a -> f b) -> (s -> f t)
data Traversal s t a b =
  Traversal (forall f. Applicative f => Optic (->) f s t a b)

----

class AsTraversal k where
  toTraversal ::
    k s t a b
    -> Traversal s t a b

instance AsTraversal Lens where
  toTraversal (Lens x) =
    Traversal x

instance AsTraversal Prism where
  toTraversal (Prism x) =
    Traversal x

----

class Compose k where
  (|.) :: k s v u t -> k u t a b -> k s v a b

instance Compose Lens where
  Lens f |. Lens g =
    Lens (f . g)

instance Compose Prism where
  Prism f |. Prism g =
    Prism (f . g)

instance Compose Traversal where
  Traversal f |. Traversal g =
    Traversal (f . g)
